  ...
  <div class="flex flex-col w-full p-6 mb-10 gap-12 lg:flex-row lg:gap-4">
    <main class="flex-1">
      <div class="flex items-center gap-4">
        <h3 class="font-semibold text-2xl text-gray-100">Favorite</h3>
        <!-- BUTTONS: Watch list Options -->
        <div class="flex items-center gap-1 text-sm">
          <button
            data-favorite="favoriteMovieList"
            ref="favoriteMovieList"
            ...
            :class="{
              'bg-violet-500 text-gray-100':
                selectedWatchList === 'favoriteMovieList',
            }"
            @click="onSelectFavorite($event)"
          >
            Movies
          </button>
          <button
            data-favorite="favoriteTvShowsList"
            ref="favoriteTvShowsList"
            class="px-2.5 py-1 rounded-md bg-gray-800 hover:bg-violet-500 hover:text-gray-100 transition-colors duration-300"
            :class="{
              'bg-violet-500 text-gray-100':
                selectedWatchList === 'favoriteTvShowsList',
            }"
            @click="onSelectFavorite($event)"
          >
            TV Shows
      ...
      <div
        ...
        v-if="selectedWatchList === 'favoriteMovieList'"
      >
      ...
      <!-- Favorite TV Shows -->
      <div
        v-if="selectedWatchList === 'favoriteTvShowsList'"
        class="flex flex-col gap-2 w-full mt-4"
      >
        Favorite TV Shows
      </div>
...
const { getFavoriteMovies, favoriteMovies, isFetching } = useWatchList();

const selectedWatchList = ref("favoriteMovieList");

const onSelectFavorite = (event) => {
  selectedWatchList.value = event.target.getAttribute("data-favorite");
};
